...
public class  TreeNode {
    ...
    private TreeNode _insert(TreeNode node, int value, int height) {
        if(node == null) {
            node = new TreeNode(value);
            node.height = height;
        }
        else if (value < node.value) {
            node.left =_insert(node.left, value, height+1);
        }
        else {
            node.right = _insert(node.right, value, height+1);
        }
        return node;
    }
    ...
    public void insert(int valueToInsert) {
        if(valueToInsert < this.value) {
            this.left = _insert(this.left, valueToInsert, 1);
        } else {
            this.right = _insert(this.right, valueToInsert, 1);
        }
    }
